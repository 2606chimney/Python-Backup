...
#### Import Tag List
df_list = pd.read_excel('Query_Tag_Lists.xlsx', sheet_name='SHL_daily')
dim = len(df_list)
...
#### Saving Weekly Data
df.to_csv('SHL_Daily_Temp_Query.csv', index=False, float_format='%.2f')
del df


#### Load Data
df = pd.read_csv('SHL_Daily_Temp_Query.csv')
...
for k in range(0, len(col_loc)):
    ...
    for i in range(1, 51):
        idx_start = num_Col * (i - 1) + col_loc[k]
        idx_end = idx_start + 1
...
df.loc[:,'Score_Gen_Brg1'] = Z.apply(lambda x: Score_Gen_Brg1(x))

## Gen Brg-2
Z = df['UMean_GnBrg2'] - df['GnBrg2']
# RiskScore
...
df.loc[:,'Score_Gen_Brg2'] = Z.apply(lambda x: Score_Gen_Brg2(x))

## ShfBrg1
Z = df['UMean_TrmTmpShfBrg1'] - df['TrmTmpShfBrg1']
# RiskScore
...
df.loc[:,'Score_TrmTmpShfBrg1'] = Z.apply(lambda x: Score_TrmTmpShfBrg1(x))

## ShfBrg2
Z = df['UMean_TrmTmpShfBrg2'] - df['TrmTmpShfBrg2']
# RiskScore
...
df.loc[:,'Score_TrmTmpShfBrg2'] = Z.apply(lambda x: Score_TrmTmpShfBrg2(x))

## GnTmpSta
Z = df['UMean_GnTmpSta'] - df['GnTmpSta']
# RiskScore
...
df.loc[:,'Score_GnTmpSta'] = Z.apply(lambda x: Score_GnTmpSta(x))

## HubTmp
Z = df['UMean_HubTmp'] - df['HubTmp']
# RiskScore
def Score_HubTmp(x):
    if x > 2.0:
        Score_HubTmp = 30.0
        ...
        Score_HubTmp = 90.0
    return Score_HubTmp
df.loc[:,'Score_HubTmp'] = Z.apply(lambda x: Score_HubTmp(x))

## RotBrgTmp
Z = df['UMean_RotBrgTmp'] - df['RotBrgTmp']
# RiskScore
...
df.loc[:,'Score_RotBrgTmp'] = Z.apply(lambda x: Score_RotBrgTmp(x))

## GbxOil
Z = df['UMean_TrmTmpGbxOil'] - df['TrmTmpGbxOil']
# RiskScore
...
df.loc[:,'Score_TrmTmpGbxOil'] = Z.apply(lambda x: Score_TrmTmpGbxOil(x))

# CnvAirTmp
Z = df['UMean_CnvAirTmp'] - df['CnvAirTmp']
# RiskScore
...
df.loc[:,'Score_CnvAirTmp'] = Z.apply(lambda x: Score_CnvAirTmp(x))


#### Save to a Desination Folder
df.to_csv('C:/Users/Chongchan.Lee/OneDrive - PIC Group, Inc/ROC/VTScadaQuery/SHL_Daily_Temp_Condition.csv', index=False, float_format='%.2f')
